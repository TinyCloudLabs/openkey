...
session key, a 256-bit device secret, and an independent PKCE verifier. It sends
only the public session binding, hashed secret, PKCE challenge, requested Share
permission, Node/Share origins, and requested expiry.
...
- Approved delegation payloads are AES-GCM encrypted at rest and consumed once
  with an atomic status transition. No relay row contains a plaintext
  delegation or CLI private key.
...
may provide a dedicated encryption root; otherwise OpenKey uses
`BETTER_AUTH_SECRET`. Production refuses to start the device route without one
of those secrets.
...
invokes the public CLI command, and verifies device prompting, cryptographic
delegation persistence, one-shot-attested uploads, seven-day retention, and the
complete Share URL.
