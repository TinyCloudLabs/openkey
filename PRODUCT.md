...
- Individual account holders using the personal OpenKey Account for keys and connected apps.
- Organization admins and members using the OpenKey Console to manage developer teams, OAuth applications, billing, rate limits, and developer audit history.
...
OpenKey exists to let users manage passkey-authenticated Ethereum key workflows and OAuth application grants without raw API calls.

The personal account surface supports identity, security, and the user-owned canonical key. The organizational console supports developer operations: creating organizations, configuring OAuth applications, managing teams, and reviewing developer audit history. Organization membership never grants access to a user's canonical key or TinyCloud data.

Success means an admin can configure an OAuth application from the browser, a member can read the same organization state without mutation controls, and sensitive one-time secrets are handled safely.
...
- Generic admin panels that treat OAuth client secrets like durable values.
...
4. Treat sensitive data as ephemeral. OAuth client secrets must be revealed intentionally and never persisted in browser storage.
