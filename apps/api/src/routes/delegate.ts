...
delegateRouter.post('/authorize-sign', async (c) => {
  ...
  if (candidateKeys.length === 0) {
    return c.json({ error: 'No managed keys found', code: 'key_not_found' }, 404);
  }
  ...
  // without them, the resulting SIWE is BROADER than the original — the
  // SDK-side `unauthorizedRecapCapabilities` check will reject the drift,
  // but relying on the client to catch server-side broadening is not
  // acceptable. Fail closed here by detecting caveats in the ReCap
  // payload of the bound original SIWE and refusing to narrow.
  if (hasRecapCaveats(bound.originalSiwe)) {
    return c.json(
      {
        error:
  ...
  // the header values captured at prepare time.
