...
export type ConsoleAppType = 'spa' | 'native';
...
export function validateConsoleRedirectUri(
  value: unknown,
  applicationType: ConsoleAppType,
  ...
  if (!PRIVATE_USE_SCHEME.test(scheme)) {
    return {
      valid: false,
...
export function validateConsoleRedirectUris(
  values: unknown,
  applicationType: ConsoleAppType,
): { valid: true } | { valid: false; reason: string } {
  ...
  for (const value of values) {
    const result = validateConsoleRedirectUri(value, applicationType);
    if (!result.valid) return result;
  }
  return { valid: true };
}

export function validateConsoleMetadataUrl(value: unknown): { valid: true } | { valid: false; reason: string } {
  if (typeof value !== 'string' || value.length === 0 || value.length > 2_048
