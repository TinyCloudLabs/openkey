...
const tc492Migrations: ReadonlyMap<string, string> = new Map([
...
export function selectProductionMigrationMode(input: {
  ...
  const unexpectedPending = pending.filter((name) => !tc492Migrations.has(name) && name !== deviceMigration);
  if (
    unexpectedPending.length > 0 ||
    !pending.includes(tc488Migration) ||
    !pending.every((name) => tc492Migrations.has(name) || name === deviceMigration)
  ) {
    throw new Error(`Pre-TC-488 production has an unreviewed pending migration set: ${pending.join(', ')}`);
  }

  for (const [name, checksum] of tc492Migrations) {
...
async function deployDeviceMigrationBeforeTc488(
  ...
  if (!alreadyApplied) {
    ...
    const parkedMigrations = pendingMigrations.filter((name) => tc492Migrations.has(name));
    ...
    try {
      ...
      // Prisma has no single-migration flag. With every pending entry and
      // checksum checked above, parking the separately authorized TC-492 set
      // makes the normal engine apply only the reviewed additive device table.
  ...
  const row = rows[0];
  if (!row?.finished_at || row.rolled_back_at || row.checksum !== deviceChecksum) {
...
async function main() {
  ...
  try {
    ...
    const alreadyApplied = state.migrations.some(
      (row) => row.migration_name === deviceMigration && row.finished_at && !row.rolled_back_at,
    );
    const successful = new Set(
      state.migrations
    ...
    await deployDeviceMigrationBeforeTc488(database, pending, alreadyApplied);
  } finally {
